...
class RingMaze:
    def __init__(self, rings, sectors):
        ...
        self.disjoint_set = DisjointSet(rings * sectors)
        self.generate_maze()

    def generate_maze(self):
        # Create a list of all walls that can be removed
        walls = self.get_all_walls()
        random.shuffle(walls)  # Randomize the walls to be removed

        for wall in walls:
            ring1, sector1, ring2, sector2 = wall
            if ring1 == ring2:  # This is a radial wall
                set1 = ring1 * self.sectors + sector1
                set2 = ring2 * self.sectors + sector2
            else:  # This is a wall between rings (angular wall)
                set1 = min(ring1, ring2) * self.sectors + sector1
                set2 = max(ring1, ring2) * self.sectors + sector1

            # Check if the two cells divided by the wall belong to different sets
            if self.disjoint_set.find(set1) != self.disjoint_set.find(set2):
                self.remove_wall(wall)
                self.disjoint_set.union(set1, set2)
        ...
        for ring in range(self.rings):
            for sector in range(self.sectors):
                ...
                if sector < self.sectors - 1:
                    walls.append((ring, sector, ring, sector + 1))
                else:
                    walls.append((ring, sector, ring, 0))  # Wrap around

                # Add walls between rings
                if ring < self.rings - 1:
                    walls.append((ring, sector, ring + 1, sector))

        return walls

    def remove_wall(self, wall):
        ring1, sector1, ring2, sector2 = wall
        if ring1 == ring2:  # Radial wall
            self.maze[ring1][sector1]['radial_walls'][1] = False
            self.maze[ring2][sector2]['radial_walls'][0] = False
        else:  # Angular wall
            self.maze[ring1][sector1]['wall'] = False
            self.maze[ring2][sector2]['wall'] = False

    def get_sector_info(self, ring, sector):
        return self.maze[ring][sector]

    def get_neighbours(self, ring, sector):
        # Calculate and return the list of neighboring sectors
        neighbours = []
        # Check the neighboring sectors within the same ring
        neighbours.append((ring, (sector - 1) % self.sectors))  # Previous sector
        neighbours.append((ring, (sector + 1) % self.sectors))  # Next sector
        # Check the sector in the inner and outer rings
        if ring > 0:
            neighbours.append((ring - 1, sector))  # Inner ring
        if ring < self.rings - 1:
            neighbours.append((ring + 1, sector))  # Outer ring
        return neighbours
        ...
        for ring in range(self.maze.rings):
            ...
            for sector in range(self.maze.sectors):
                ...
                if sector_info['radial_walls'][0]:
                    # print(f"Drawing start radial wall at ({ring},{sector})")
                ...
                if sector_info['radial_walls'][1]:
                    # print(f"Drawing end radial wall at ({ring},{sector})")
